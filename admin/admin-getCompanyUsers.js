...
getCompanyUsers.post('/',verifyToken, function (req, res, next) {
    //DB Connection
    try{
        const db = req.admin_db;
        const coll = db.collection('User');
        var companynames = new Array();
        // console.log("JSON.parse(req.query.companyname)",JSON.parse(req.query.companyname));
      ...
      /* To Fetch the username for the selected company from 'user' collection */
       ...
       var query = [
            ...
            },
            {
                "$project":
                    {
                        'ContactDetails.EmailID': 1,'UserName': 1,'UserType':1, "_id": 0, 'UserStatus':1, 'CompanyName':1   //username
                    }
            }
        ]
    }else{
        var query = [
            {
                $match: {
                    'RegisteredCompanyName': { $in: companynames },
                    'UserName': { $ne: null },    //added
            ...
            },
            {
                "$project":
                    {
                        'ContactDetails.EmailID': 1,'UserName': 1,'UserType':1, "_id": 0, 'UserStatus':1,'UserID':1,'RegisteredCompanyName':1,   //username
                    }
            }
        ]
    }
